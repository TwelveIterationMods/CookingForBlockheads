...
public class SinkRenderer extends TileEntityRenderer<SinkTileEntity> {
    ...
    public void render(SinkTileEntity tileEntity, float partialTicks, MatrixStack matrixStack, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay) {
        ...
        if (waterAmount > 0) {
            ...
            dispatcher.getBlockModelRenderer().renderModel(world, ModModels.sinkLiquid, tileEntity.getBlockState(), tileEntity.getPos(), matrixStack, buffer.getBuffer(RenderType.translucent()), false, world.rand, 0, Integer.MAX_VALUE, EmptyModelData.INSTANCE);
            matrixStack.pop();
        }
    }
