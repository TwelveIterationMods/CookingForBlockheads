...
public class SinkRenderer implements BlockEntityRenderer<SinkBlockEntity> {
    ...
    public void render(SinkBlockEntity blockEntity, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay, Vec3 cameraPos) {
        ...
        if (waterAmount > 0) {
            ...
            dispatcher.getModelRenderer().tesselateBlock(level, ModModels.sinkLiquid.get().collectParts(random), blockEntity.getBlockState(), blockEntity.getBlockPos(), poseStack, buffer.getBuffer(RenderType.translucent()), false, Integer.MAX_VALUE);
            poseStack.popPose();
        }
    }
