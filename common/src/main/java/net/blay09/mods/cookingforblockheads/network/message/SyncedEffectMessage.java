...
public class SyncedEffectMessage {
    ...
    public static void handle(Player player, SyncedEffectMessage message) {
        ...
        if (soundEvent != null) {
            Minecraft.getInstance().level.playLocalSound(message.pos.getX(), message.pos.getY(), message.pos.getZ(), soundEvent, SoundSource.BLOCKS, volume, 1f, false);
        }
        ...
        for (int i = 0; i < particleCount; i++) {
            ...
            Minecraft.getInstance().level.addParticle(particleType, true, message.pos.getX() + offsetX, message.pos.getY() + offsetY, message.pos.getZ() + offsetZ, 0f, 0f, 0f);
        }
    }
