...
public class DoorAnimator {
    ...
    public void update() {
        ...
        if ((isForcedOpen || numPlayersUsing > 0) && angle == 0f && soundEventOpen != null) {
            blockEntity.getLevel().playLocalSound(x, y, z, soundEventOpen, SoundSource.BLOCKS, 0.5f, blockEntity.getLevel().getRandom().nextFloat() * 0.1f + 0.9f, false);
        }
        ...
        if (((numPlayersUsing == 0 || !isForcedOpen) && angle > 0f) || ((isForcedOpen || numPlayersUsing > 0) && angle < 1f)) {
            ...
            if (angle < playCloseSound && angleBefore >= playCloseSound && soundEventClose != null) {
                blockEntity.getLevel().playLocalSound(x, y, z, soundEventClose, SoundSource.BLOCKS, 0.5f, blockEntity.getLevel().getRandom().nextFloat() * 0.1f + 0.9f, false);
            }
