...
public class ItemIceUnit extends Item {
    ...
    public ActionResultType onItemUseFirst(ItemStack stack, ItemUseContext context) {
        ...
        if (tileEntity instanceof FridgeTileEntity && !((FridgeTileEntity) tileEntity).getBaseFridge().hasIceUpgrade) {
            if (!player.abilities.isCreativeMode) {
                player.getHeldItem(context.getHand()).shrink(1);
            }

            ((FridgeTileEntity) tileEntity).getBaseFridge().setHasIceUpgrade(true);
            if (!world.isRemote) {
